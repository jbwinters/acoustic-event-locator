...
def require_ffmpeg():
    for binname in ("ffmpeg", "ffprobe"):
        if shutil.which(binname) is None:
            log(
                f"Required tool '{binname}' is not on PATH. Please install ffmpeg.",
                "ERROR",
            )
            sys.exit(2)
...
def extract_audio_ffmpeg(in_video: str, out_wav: str, target_sr=48000):
    ...
    try:
        ...
        log(
            f"ffmpeg failed on {in_video} with error:\n{e.stderr.decode('utf-8', errors='ignore')}",
            "ERROR",
        )
        raise
...
def load_positions(positions_json: str, videos_dir: str):
    ...
    for m in J["mics"]:
        ...
        if not os.path.exists(p):
            ...
            sys.exit(2)
        mics.append(
            Mic(file=p, lat=m["lat"], lon=m["lon"], height_m=m.get("height_m", 0.0))
        )
    # reference lat/lon
...
def extract_all_audio(
    mics: List[Mic], out_dir: str, fs=48000
...
def pick_arrival_indices(
    x: np.ndarray, fs: int, band=(200, 4000)
    ...
    if krel is None:
        krel = sta_lta_picker(segment, fs, sta_ms=5.0, lta_ms=200.0, thr=3.5) or int(
            0.5 * (right - left)
        )
    k = int(left + krel)
    ...
    snr_like = float(
        (np.max(env[k : k1 + 1]) - np.median(env[k0:k])) / (baseline + 1e-9)
    )
    return k, float(k) / fs, snr_like


def refine_arrivals_with_template(
    arrivals: List[int],
    signals_filt: List[np.ndarray],
    fs: int,
    pre_ms=40.0,
    post_ms=70.0,
) -> List[float]:
    ...
    for i in range(M):
        k = arrivals[i]
        a = max(0, k - pre)
        b = min(len(signals_filt[i]), k + post)
    ...
    a = max(0, ref_k - pre)
    b = min(len(signals_filt[ref_idx]), ref_k + post)
    ...
    for i in range(M):
        for j in range(i + 1, M):
            ...
            if abs(tij) > max_tdoa:
                ...
                log(
                    f"Dropping pair ({i},{j}) |t_ij|={tij:.4f}s > max {max_tdoa:.4f}s",
                    "WARN",
                )
                continue
...
def alternation_solver(
    XY, pairs, z, w_base, c, max_outer=20, huber_k_ms=2.0, grid_res_m=2.0
):
    """
    Alternate:
...
def plot_layout(XY, s_xy, cov, out_png: str):
    ...
    R = np.array(
        [
            [np.cos(np.deg2rad(ang_deg)), -np.sin(np.deg2rad(ang_deg))],
...
def write_sync_csv(
    out_csv: str, files: List[str], arrival_s: List[float], delta: np.ndarray
):
    """
    ...
    with open(out_csv, "w", newline="") as f:
        w = csv.writer(f)
        w.writerow(
            ["file", "arrival_time_s", "clock_offset_s", "align_to_event_offset_s"]
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Locate a single impulsive acoustic event from multiple videos."
    )
    parser.add_argument(
        "--videos_dir", required=True, help="Directory containing input videos."
    )
    parser.add_argument(
        "--positions", required=True, help="positions.json mapping file->lat/lon."
    )
    parser.add_argument("--out", default="./out", help="Output directory.")
    parser.add_argument(
        "--fs", type=int, default=48000, help="Target sample rate for audio extraction."
    )
    parser.add_argument(
        "--bandpass",
        type=float,
        ...
        help="Bandpass (low high) Hz.",
    )
    parser.add_argument(
        "--grid_res_m",
        type=float,
        default=2.0,
        help="Grid resolution (m) for initialization.",
    )
    parser.add_argument(
        "--huber_k_ms",
        type=float,
        default=2.0,
        help="Huber threshold (ms) for robust weighting.",
    )
    parser.add_argument(
        "--assume_3d",
        action="store_true",
        help="If set, treat heights as meaningful (z used for distance); still solves in 2D for location.",
    )
    ...
    if M < 3:
        log(
            "Need at least 3 videos (≥4 if devices are unsynchronized) to localize robustly.",
            "ERROR",
        )
        sys.exit(2)
    if M < 4:
        log(
            "WARNING: With 3 unsynchronized devices and a single event, the problem can be underdetermined.",
            "WARN",
        )
    ...
    for (wav_path, fs2, x), m in zip(wavs, mics):
        xf = apply_bandpass(
            x, fs, low_hz=args.bandpass[0], high_hz=args.bandpass[1], order=4
        )
    ...
    arrivals_s_ref = refine_arrivals_with_template(
        arrivals_idx, filtered_signals, fs, pre_ms=40.0, post_ms=70.0
    )
    for i in range(M):
    ...
    if len(pairs) < M - 1:
        log(
            "Too few valid TDOA pairs after gating; try relaxing bandpass, or ensure event is clearly audible.",
            "ERROR",
        )
